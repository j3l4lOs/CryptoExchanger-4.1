...
$latest_news = '';
$NewsQuery = $db->query("SELECT * FROM ce_news ORDER BY id DESC LIMIT 3");
if($NewsQuery->num_rows>0) {
    while($news = $NewsQuery->fetch_assoc()) {
        ...
        $newtpl->set("date",date("d/m/Y",$news['created']));
        $latest_news .= $newtpl->output();
    }
}
$tpl->set("latest_news",$latest_news);
$reviews = '';
$ReviewsQuery = $db->query("SELECT * FROM ce_users_reviews WHERE status='1' ORDER BY RAND() LIMIT 3");
if($ReviewsQuery->num_rows>0) {
    while($review = $ReviewsQuery->fetch_assoc()) {
        ...
        $retpl->set("date",date("d/m/Y",$review['posted']));
        $reviews .= $retpl->output();
    }
}
$tpl->set("reviews",$reviews);
$latest_exchanges = '';
$ExchangesQuery = $db->query("SELECT * FROM ce_orders WHERE status='4' ORDER BY id DESC LIMIT 2");
